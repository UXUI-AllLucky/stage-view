...
const Play = ({ WorkData }) => {
    ...
    // 2. 훅에는 전체 데이터(WorkData)가 아니라, 걸러낸 데이터(playListData)를 줍니다.
    // Hook
    // hook에 적용하는 data이름 playList로 변경함
    const { data: playList, stateMenu, sortMenu, onShow, onSort } = useContentControl(playListData);
    ...
    return (
        <div className="play-inner">
            ...
            {/* playwork={playwork} -> playwork={playList} 로 변경 */}
            <PlayContent playwork={playList} />
        </div>
    );
};
